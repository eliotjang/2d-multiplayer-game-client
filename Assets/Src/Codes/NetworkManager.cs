...
public class NetworkManager : MonoBehaviour
{
    ...
    void ProcessResponseData(byte[] data, uint handlerId) {
        try {
            ...
            float lastPositonX = (float)jsonObject["x"];
            float lastPositonY = (float)jsonObject["y"];
            uint playerId = (uint)jsonObject["playerId"];

            Debug.Log(lastPositonX);
            Debug.Log(lastPositonY);
            Debug.Log(playerId);
            ...
            if (handlerId == 0)
            {
                // 클라이언트 게임 시작
                Debug.Log("Game Started");
                GameManager.instance.GameStart(lastPositonX, lastPositonY, playerId);
            }
        } catch (Exception e) {
